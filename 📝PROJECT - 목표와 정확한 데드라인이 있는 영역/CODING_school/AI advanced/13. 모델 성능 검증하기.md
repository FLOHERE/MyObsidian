...
![[Pasted image 20250622110535.png]]

- 모델 성능 향상 방법
	1. 데이터를 보강하는 방법
	2. 가지고 있는 데이터를 적절히 보완해주는 방법
		1. 충분한 양의 데이터 추가 , 샘플수가 많을수록 성능이 좋아진다.
		2. 테이블형 데이터의 경우 너무 크거나, 낮은 이상치가 모델에 영향을 줄 수 없도록 크기를 적절히 조절 할수 있다.
		3. sigmoid 함수를 이용 -> 전체를 0~1 사이의 값으로 변환하는것도 좋음
		4. 교차 검증 방법을 이용해 가지고 있는 데이터를 충분히 이용하는 방법
	3. 알고리즘을 최적화 하는 방법
		1. 다른 구조로 모델을 바꾸어 가며 최적의 구조를 찾는다.
			- 은닉층의 개수, 
			- 그 안에 들어갈 노드 수, 
			- 최적화 함수의 종류를 바꾸는 등
		2. 자신의 데이터에 꼭 맞는 구조를 계속해서 테스트 해보며 찾는것이 중요
		3. 더 나은 알고리즘으로 바꾸는 방법 등


![[Pasted image 20250622111221.png]]

## 4. 모델 저장과 재사용
- 학습이 끝난 후 지금 만든 모델을 저장하면 언제든 이를 불러와 다시 사용 가능
- `model.save()` : 학습 결과 저장 함수
...
```
 ...
 - *.hdf5* 파일 포멧 : 주로 과학 기술데이터 작업에서 사용됨
	 - 크고 복잡한 데이터를 저장하는데 사용
- `load_model`(케라스 API) : 조금 전 저장한 모델 불러옴.
- `del` : 모델의 메모리 삭제 함수

```python
# 모델이 저장된 위치와 이름까지 적어줌
...
```python
score = model.evaluate(X_test, y_test) # 테스트셋에 모델 적용시
print('Test accuracy', score[1]) # loss 와 accuracy 값을 모두 출력한다.
```

- 이전에 실행한 것과 같은 값이 나온다.

![[Pasted image 20250622114326.png]]

## 5. k겹 교차 검증
- 모델 만들때 학습데이터가 많이 필요함(big data)
	- 근데 이 빅데이터가 많이 없다!
	- 데이터가 없을때 대용으로 기존의 데이터를 재 활용하는 방식이 k 교차 검증!
- k겹 교차 검증이란?
	- 데이터셋을 여러개로 나누어 하나씩 테스트셋으로 사용하고 나머지를 모두 합해서 학습셋으로 사용하는 방법
- `KFold()` : 데이터를 각각 원하는 수만큼 나누어 학습셋과 데이터셋으로 사용되기 하는 사이킷런 라이브러리의 함수

![[Pasted image 20250622114739.png]]
